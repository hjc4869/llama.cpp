...
struct llama_file::impl {
    impl(const char * fname, const char * mode) {
        ...
        local_read_buf = std::vector<no_init<uint8_t>>(8192);
        aligned_tempbuf = local_read_buf.data() + (4096 - (int64_t)local_read_buf.data() % 4096);
    }

    size_t tell() const {
        off_t ret = ::lseek(fd, 0, SEEK_CUR);
        if (ret == -1) {
            throw std::runtime_error("lseek error: " + std::string(strerror(errno)));
        }
    ...
    void read_raw(void * ptr, size_t len) const {
        ...
        // auto unaligned_off = (uint64_t)ptr % 4096;
        // if (unaligned_off > 0) {
        //     auto prologue_len = 4096 - unaligned_off;
        //     ssize_t ret = read(fd, aligned_tempbuf, prologue_len);
        //     if (ret == -1) {
        //         throw std::runtime_error("read error: " + std::string(strerror(errno)));
        //     } 
            
        //     if (static_cast<size_t>(ret) != prologue_len) {
        //         throw std::runtime_error("Unexpectedly reached end of file");
        //     }

        //     memcpy(ptr, aligned_tempbuf, prologue_len);
        //     ptr = (unsigned char*)ptr + prologue_len;
        //     len -= prologue_len;
        // }
    ...
    void * aligned_tempbuf;
    std::vector<no_init<uint8_t>> local_read_buf;
    int fd;
    size_t size;
};
