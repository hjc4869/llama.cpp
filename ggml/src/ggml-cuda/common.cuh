...
#if (__CUDA_ARCH__ < GGML_CUDA_CC_OFFSET_AMD && __CUDA_ARCH__ >= GGML_CUDA_CC_VOLTA) || defined(RDNA3) || defined(CDNA)
#define FP16_MMA_AVAILABLE
#endif //  (__CUDA_ARCH__ < GGML_CUDA_CC_OFFSET_AMD && __CUDA_ARCH__ >= GGML_CUDA_CC_VOLTA) || defined(RDNA3) || defined(CDNA)
...
static bool fp16_mma_available(const int cc) {
    return cc < GGML_CUDA_CC_OFFSET_AMD && ggml_cuda_highest_compiled_arch(cc) >= GGML_CUDA_CC_VOLTA || cc >= GGML_CUDA_CC_CDNA && cc < GGML_CUDA_CC_RDNA1 || cc >= GGML_CUDA_CC_RDNA3 && cc <= GGML_CUDA_CC_RDNA4;
}

// To be used for feature selection of external libraries, e.g. cuBLAS.
static bool fp16_mma_hardware_available(const int cc) {
    return cc < GGML_CUDA_CC_OFFSET_AMD && cc >= GGML_CUDA_CC_VOLTA || cc >= GGML_CUDA_CC_CDNA && cc < GGML_CUDA_CC_RDNA1 || cc == GGML_CUDA_CC_RDNA3 && cc <= GGML_CUDA_CC_RDNA4;
}
